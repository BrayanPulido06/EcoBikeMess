document.addEventListener('DOMContentLoaded', function() {
    ...
    const comprobantesGrid = document.getElementById('comprobantesGrid');
    ...
    function renderComprobantes() {
        ...
        // Vista Grid
        comprobantesGrid.innerHTML = paginatedComprobantes.map(comp => `
            <div class="comprobante-card" data-id="${comp.id}">
                <div class="card-header-section">
                    <div class="guia-badge">${comp.guia}</div>
                    <div class="status-badge status-${comp.estado}">
                        ${comp.estado === 'entregado' ? '✓ Entregado' : '⏳ Pendiente'}
                    </div>
                </div>
                <div class="card-info">
                    <div class="info-line">
                        <span class="label">Destinatario:</span>
                        <span class="value">${comp.destinatario}</span>
                    </div>
                    <div class="info-line">
                        <span class="label">Recibió:</span>
                        <span class="value">${comp.quienRecibio}</span>
                    </div>
                    <div class="info-line">
                        <span class="label">Fecha:</span>
                        <span class="value">${formatDate(comp.fechaEntrega)}</span>
                    </div>
                    <div class="info-line">
                        <span class="label">Recaudo:</span>
                        <span class="value">${comp.recaudo > 0 ? '$' + comp.recaudo.toLocaleString('es-CO') : 'Sin recaudo'}</span>
                    </div>
                </div>
                <img src="${comp.foto}" alt="Evidencia" class="card-image">
                <div class="card-actions">
                    <button class="btn-action" onclick="verComprobante(${comp.id})">👁️ Ver</button>
                    <button class="btn-action primary" onclick="descargarPDF(${comp.id})">⬇️ PDF</button>
                </div>
            </div>
        `).join('');
        ...
        comprobantesTableBody.innerHTML = paginatedComprobantes.map(comp => `
            <tr onclick="verComprobante(${comp.id})">
                ...
                <td class="table-actions">
                    <button class="icon-btn" onclick="event.stopPropagation(); verComprobante(${comp.id})" title="Ver">👁️</button>
                    <button class="icon-btn" onclick="event.stopPropagation(); descargarPDF(${comp.id})" title="Descargar">⬇️</button>
                    <button class="icon-btn" onclick="event.stopPropagation(); imprimirComprobante(${comp.id})" title="Imprimir">🖨️</button>
                </td>
            </tr>
        `).join('');
    ...
    function applyFilters() {
        ...
        filteredComprobantes = comprobantes.filter(comp => {
            const matchSearch = comp.guia.toLowerCase().includes(searchTerm) ||
                              ...
                              comp.quienRecibio.toLowerCase().includes(searchTerm);
    ...
    window.descargarPDF = async function(id) {
        ...
        try {
            ...
            doc.text(`Nombre: ${comp.cliente}`, margin + 5, yPos);
            yPos += 6;
            doc.text(`Dirección: ${comp.direccion}`, margin + 5, yPos);
            yPos += 12;
            ...
            // Nota: La foto se agregaría aquí con html2canvas en una implementación real
            doc.setFontSize(10);
            doc.setTextColor(100, 100, 100);
            ...
            doc.setTextColor(92, 184, 92);
            doc.text('Válido por 1 año desde la fecha de entrega', margin, 280);
    ...
    toggleBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (view === 'grid') {
                comprobantesGrid.classList.add('active');
                comprobantesList.classList.remove('active');
            } else {
                comprobantesList.classList.add('active');
                comprobantesGrid.classList.remove('active');
            }
        });
    });
