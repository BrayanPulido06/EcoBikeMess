document.addEventListener('DOMContentLoaded', function() {
    ...
    function cargarEstadisticas() {
        ...
        fetch(`../../controller/facturacionClienteController.php?${params.toString()}`)
            .then(res => res.json())
            .then(response => {
        ...
        data.forEach(f => {
            ...
            html += `
                <tr>
                    ...
                    <td>${f.fecha_creacion.substring(0, 10)}</td>
                    <td>${f.destinatario_nombre}</td>
                    <td>${f.direccion_destino}</td>
                    <td style="font-weight:bold;">${formatCurrency(f.costo_envio)}</td>
